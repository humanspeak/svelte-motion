...
| 004  | Motion-dom projection authoritative (page-space, retire legacy FLIP) | P1         | L      | 003        | **YES — update** `docs/layout-animations`; shared-layout page as follow-up | IN PROGRESS — Steps 1-6 executed + docs updated; ONE STOP: deleting `projection.ts` blocked by out-of-scope type-only imports (Reorder `Axis`/`Box`) — legacy node fully unused at runtime, `projection.context.ts` deleted; awaiting guard close-out |
