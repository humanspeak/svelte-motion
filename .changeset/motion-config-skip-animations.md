...
Unlike `reducedMotion="always"`, which strips only transform keys while opacity and colour keep animating, `skipAnimations` disables the tween for every animated property. Layout/FLIP projection animations and drag momentum are unaffected, matching upstream. The prop is reactive — toggling it retargets already-mounted elements without remounting the subtree.
