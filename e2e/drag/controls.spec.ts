...
test.describe('drag/controls', () => {
    test('imperative start moves x only with dragControls', async ({ page }) => {
        ...
        await handle.dispatchEvent('pointerdown', { clientX: s.x - 20, clientY: s.y + 10, pointerId: 1 })
        await page.dispatchEvent('body', 'pointermove', { clientX: s.x + 60, clientY: s.y + 40, pointerId: 1 })
        await page.dispatchEvent('body', 'pointerup', { clientX: s.x + 60, clientY: s.y + 40, pointerId: 1 })
