...
const CARD_DIMS: Record<CardType, { width: number; height: number }> = {
...
function buildOgHtml(opts: {
    ...
    <div style="display: flex; position: absolute; top: -10rem; right: -10rem; height: 40rem; width: 40rem; border-radius: 9999px; background-image: radial-gradient(circle, rgba(61, 187, 160, 1) 0%, rgba(61, 187, 160, 0) 70%); opacity: 0.3;"></div>
    ...
    <div style="display: flex; position: relative; height: 100%; flex-direction: column; justify-content: space-between;">
        <div style="display: flex; flex-direction: column; gap: 0.75rem;">
            <div style="display: flex; align-items: center; align-self: flex-start; gap: 0.5rem; border-radius: 9999px; border: 1px solid rgba(255, 255, 255, 0.1); background-color: rgba(255, 255, 255, 0.05); padding: 0.5rem 1rem; font-size: 0.875rem; line-height: 1.25rem; font-weight: 600; letter-spacing: 0.025em; color: rgba(255, 255, 255, 0.8); text-transform: uppercase; font-family: 'lato';">
                ...
                <span>${docsConfig.npmPackage}</span>
            </div>
        ...
        <div style="display: flex; width: 100%; align-items: center; justify-content: space-between;">
            ...
            <div style="display: flex; align-items: center; gap: 1rem; border-radius: 1rem; border: 1px solid rgba(255, 255, 255, 0.1); background-color: rgba(255, 255, 255, 0.05); padding: 1rem; padding-right: 1.5rem; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 8px 10px -6px rgba(0, 0, 0, 0.1);">
                ...
                <div style="display: flex; flex-direction: column;">
                    <span style="font-size: 0.75rem; line-height: 1rem; font-weight: 600; letter-spacing: 0.05em; color: #3dbba0; text-transform: uppercase; font-family: 'lato';">Built for</span>
...
async function generateCard(
    ...
    const dims = CARD_DIMS[task.type]
    ...
    await fs.writeFile(path.join(ROOT, 'static/social-cards', task.filename), png)
}
...
async function main() {
    ...
    // Ensure output directory exists once before generating cards
    await fs.mkdir(path.join(ROOT, 'static/social-cards'), { recursive: true })

    // Build task list: per-page og + twitter variants, plus defaults
    const defaultTitle = docsConfig.name
    const defaultDescription =
        'Spring physics, gestures, layout animations, exit animations, and scroll effects with a familiar declarative API.'
    const defaultFeatures = docsConfig.defaultFeatures
