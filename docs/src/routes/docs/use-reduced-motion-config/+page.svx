...
<script>
  ...
  import UseReducedMotionConfigExample from '$lib/examples/use-reduced-motion-config/demos/Default.svelte';
  import { getSeoContext } from '$lib/components/contexts/Seo/Seo.context'

  const seo = getSeoContext()
  if (seo) {
      seo.title = 'useReducedMotionConfig | Docs | Svelte Motion'
      seo.description = "Combine MotionConfig.reducedMotion with the user's OS preference. Reactive { current } + .subscribe shim via Svelte 5 runes."
      seo.ogTitle = 'useReducedMotionConfig'
      seo.ogTagline = 'Resolved reduced-motion policy from context + OS pref'
      seo.ogFeatures = ['Accessibility', 'Svelte 5 Runes', 'MotionConfig Aware', 'SSR Safe']
...
<Example isSmall exampleUrl="/examples/use-reduced-motion-config">
  <UseReducedMotionConfigExample />
</Example>
