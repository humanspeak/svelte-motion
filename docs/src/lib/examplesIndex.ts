...
/**
 * Metadata for one example page, keyed by slug in {@link EXAMPLES}.
 */
...
/**
 * A resolved example page: slug + route joined with its display metadata,
 * in the site's canonical (sitemap, alphabetical-by-slug) order.
 */
export type ExampleListing = {
    /** Route path, e.g. `/examples/grid-exit`. */
    route: string
    /** Directory slug, e.g. `grid-exit`. */
    slug: string
    /** Display title. */
    title: string
...
/**
 * List every example page in canonical order — the sitemap manifest's
 * alphabetical `/examples/<slug>` routes joined with {@link EXAMPLES}
 * metadata. This is the single ordering shared by the examples index
 * grid and the per-page prev/next pager, so `№` numbering always agrees.
 *
 * @returns Ordered example listings.
 * @example
 * ```ts
 * const examples = listExamples()
 * examples[0].route // '/examples/ai-glow-border'
 * ```
 */
export const listExamples = (): ExampleListing[] =>
    Object.keys(sitemapManifest)
        .filter((route) => route.startsWith('/examples/') && route !== '/examples')
        .sort()
        .map((route) => {
            const slug = route.replace('/examples/', '')
            const entry = EXAMPLES[slug]
            const title = entry?.title || formatTitle(slug)
            return {
                route,
                slug,
                title,
                description:
                    entry?.description || `Interactive ${title.toLowerCase()} animation example`
            }
        })
